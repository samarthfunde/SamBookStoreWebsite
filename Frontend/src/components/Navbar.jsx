...
function Navbar() {
    //we write following js code for the light button
    const [theme, setTheme] = useState(localStorage.getItem("theme")?localStorage.getItem("theme"): "light")
    const element= document.documentElement;
    useEffect(()=>{ //we use useEffect for the show effect only
        if(theme === "dark"){
    ...
    return (
        <>
            <div className={`max-w-screen-2xl container mx-auto fixed top-0 left-0 right-0 md:px-20 px-4 bg-white z-50 dark:bg-slate-800 dark:text-white dark:border-bottom ${sticky ? "sticky-navbar shadow-md bg-base-200 duration-300 transition-all ease-in-out " : ""} `}>
                    ...
                    <div className="navbar-start ">
                        <div className=" dropdown relative ">
                            <div
                                ...
                                onClick={() => {
                                    if (!isSmallScreen) {
                                        // Toggle dropdown visibility only on small screens
                                        return;
                                    }
                            ...
                            >
                                <IoBookSharp className="h-8 w-8 text-pink-500 " />
                            </div>
                            {isSmallScreen && (
                                <ul
                                    tabIndex={0}
                                    className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 bg-white dark:bg-slate-900 dark:text-white  "
                                >
                                    {NavbarItems}
                                </ul>
                            )}
                        </div>
                        <a className="text-2xl font-bold cursor-pointer">SamBookStore</a>
                    </div>
                    ...
                    <div className="navbar-end space-x-3 ">
                        ...
                        <div className='hidden md:block'>
                            <label className="px-3 py-2 border rounded-md flex items-center gap-2 bg-white ">
                                <input type="text" className="grow outline-none bg-white dark:white-slate-900 dark:text-black " placeholder="Search" />
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4 opacity-70 dark:black-slate-900 dark:text-black"><path fillRule="evenodd" d="M9.965 11.026a5 5 0 1 1 1.06-1.06l2.755 2.754a.75.75 0 1 1-1.06 1.06l-2.755-2.754ZM10.5 7a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0Z" clipRule="evenodd" /></svg>
                            </label>
                        </div>
                        ...
                        <div className="">
                        {/* //show model hamare liye model ko use krega */}
